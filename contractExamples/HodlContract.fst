...
let getReturnAddress(): option lock `cost` 35 = //3
    let pkHash = metavcoin.Crypto.parseHash returnAddress in //32
...
let deposit (txSkel: txSkeleton) (contractId: contractId)
                  : CR.t `cost` 182 = //16
    //let! contractToken = metavcoin.Asset.getDefault contractId in //64
    let! amount = Tx.getAvailableTokens metavcoinAsset txSkel in //64
    let! returnAddress = getReturnAddress() in //35
    match returnAddress with
    | Some returnAddress ->
        Tx.lockToContract metavcoinAsset amount contractId txSkel //64
...
let withdraw (txSkel: txSkeleton) ({timestamp=timestamp}: context)
             ...
             (wallet: wallet): CR.t `cost` (metavcoin.Wallet.size wallet * 128 + 410) = //39
    //let! contractToken = metavcoin.Asset.getDefault contractId in //64
    ...
    | Some withdrawAmount, Some returnAddress ->
        if timestamp >=^ maturityTimestamp then begin
            let! txSkel = //Tx.destroy amount contractToken txSkel //64
                          //>>= Tx.lockToAddress metavcoinAsset amount returnAddress //64
                          Tx.lockToAddress metavcoinAsset withdrawAmount returnAddress txSkel //64
                          >>= Tx.fromWallet metavcoinAsset withdrawAmount contractId wallet in //w*128+192
            match txSkel with
...
let main (txSkel: txSkeleton) (context: context) (contractId: contractId)
         ...
         : CR.t `cost` (metavcoin.Wallet.size wallet * 128 + 418) = //8
    match command with
