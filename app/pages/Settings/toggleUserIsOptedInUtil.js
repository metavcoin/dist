...
const toggleUserIsOptedInUtil = async (nextValue) => {
  if (nextValue) {
    ...
    await swal({
      text: 'Thank you for helping us improve metavcoin wallet!',
      icon: 'success',
    })
    return
  }
  const optOut = await swal({
    ...
    icon: 'warning',
    text: 'Sending error reports is 100% Anonymous and helps us improve metavcoin wallet for you and fellow users.',
    buttons: {
