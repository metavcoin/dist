...
import { getmetavcoinNodePath } from '../metavcoinNode'
...
function validateOs() {
  if (arch() !== 'x64') {
    const msg = 'metavcoin-node can only run on 64 bit OS'
    console.log(msg)
    dialog.showErrorBox(
...
function getZ3PathForPlatform() {
  const pathToDir = path.join(getmetavcoinNodePath(), 'Release')
  let filename
