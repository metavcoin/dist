...
import { Data, Chain } from '@metavcoin/metavcoinjs'
import { Spend } from '@metavcoin/metavcoinjs/build/src/Consensus/Types/Spend'
import { Payout, Recipient } from '@metavcoin/metavcoinjs/build/src/Consensus/Types/Payout'
import { Ballot } from '@metavcoin/metavcoinjs/build/src/Consensus/Types/Ballot'
import Address from '@metavcoin/metavcoinjs/build/src/Components/Wallet/Address'
import { ContractId } from '@metavcoin/metavcoinjs/build//src/Consensus/Types/ContractId'
import { Asset } from '@metavcoin/metavcoinjs/build/src/Consensus/Types/Asset'
...
export const getAssetName = (asset: ?string) => {
  if (asset === metavcoin_ASSET_HASH) { return metavcoin_ASSET_NAME }
...
export const format = (balance) =>
  (balance >= kalapasTometavcoin(1) ? kalapasTometavcoin(balance) : balance)

export const formatDisplay = (balance) =>
  (balance >= 1 ?
    toDisplay(kalapasTometavcoin(balance), 2) : toDisplay(balance, 2))
...
export const getPayoutRecord = (chain, pastBallotId) => {
  ...
  const assets = spends ? spends.map(spend => {
    const { asset, amount } = spend
    return `Asset: ${ismetavcoinAsset(asset.asset) ? 'ZP' : truncateString(asset.asset)}, Amount: ${amount >= metavcoinToKalapas(1) ? formatDisplay(amount) : toDisplay(amount)} \n`
  }) : ''
